...
DevBoard backend is a production-oriented REST API for a Jira/Trello-style developer task board. It uses Express, MongoDB, Redis, JWT auth, structured logging, Swagger docs, and Jest/Supertest test coverage.
...
The same commands are also available from the repository root with `backend:` prefixes, for example `npm run backend:test`.
...
For Docker runs, Compose maps host `5001` to container `5000`.
